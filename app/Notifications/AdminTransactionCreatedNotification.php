...
class AdminTransactionCreatedNotification extends Notification
{
    ...
    public function __construct($transaction, $user, $business)
    {
        ...
        $this->first_message = "A business has just submitted a new transaction:<br>";
        ...
        $this->first_message .= "<b>Amount to Pay: </b>".$transaction->toCurrency->code .  number_format($transaction->amount_to_pay, 2) ."<br>";
        $this->first_message .= "Please review and approve/reject in the admin panel.";

        $this->closing_message = 'Thank you for using ' . env('APP_NAME') . '!';
        $this->action_button = [['Review Transaction', url(route('admin.transactions.index') . '?trans_id=' . $transaction->trans_id)]];
    }
