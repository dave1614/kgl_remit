...
const bonus_not_withdrawable = ref(page.props.bonus_not_withdrawable);
console.log(bonus_not_withdrawable.value)
...
const totalEarnings = computed(() => {
    return (
        parseFloat(user.value.total_commission)
    ).toFixed(2);
});

const walletBalance = computed(() => {
    return (
        parseFloat(user.value.vtu_earnings) +
        parseFloat(user.value.upline_earnings) -
        (
            parseFloat(user.value.earnings_withdrawn) +
            parseFloat(user.value.total_maint_fee)
        ).toFixed(2)
    );
});
</script>
    ...
    <LayoutAuthenticated>
        ...
        /> -->
            ...
            <div class="my-6" v-if="vtu_services.length > 0">
                ...
                <SectionTitleLineWithButton :icon="mdiHumanGreetingProximity" title="Vtu Services" main>
                ...
                <div class="sm:grid sm:grid-cols-12 sm:gap-6">
                    <CardBox @click="openPage(service.route)" v-for="(service, index) in vtu_services" :key="index"
                        class="cursor-pointer sm:col-span-4 relative my-5 py-4 mx-4 shadow-xl hover:shadow-2xl transition-all ease-in-out duration-500 hover:my-5">
