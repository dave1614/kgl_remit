        ...
        onSuccess: (page) => {
            ...
            if (response.success && response.order_id !== "") {
                ...
                Swal.fire({
                    title: 'Info',
                    icon: 'info',
                    html: `You Have Successfully Credited <em class='text-emerald-500'>${phone_number}</em> With Airtime Worth ₦${mainStore.addCommas(amount)}. Note You Have Been Debited Of ₦${mainStore.addCommas(amount)}. The Order Id For This Transaction Is <em class='text-emerald-500'>"${order_id}</em>`,
        ...
        onSuccess: (page) => {
            ...
            if (response.success && response.order_id !== "") {
                ...
                Swal.fire({
                    title: 'Info',
                    icon: 'info',
                    html: `You Have Successfully Credited <em class='text-emerald-500'>${phone_number}</em> With Airtime Worth ₦${mainStore.addCommas(amount)} On <span class='capitalize'>${network}</span> Network. Note You Have Been Debited Of ₦${mainStore.addCommas(response.amount_debited)}. The Order Id For This Transaction Is <em class='text-emerald-500'>"${order_id}</em>`,
                    allowEscapeKey: false,
                    allowOutsideClick: false,
        ...
        onSuccess: (page) => {
            ...
            if (response.success) {
                ...
                Swal.fire({
                    title: 'Info',
                    icon: 'info',
                    html: `You Have Successfully Credited <em class='text-emerald-500'>${phone_number}</em> With Airtime Worth ₦${mainStore.addCommas(amount)} On <span class='capitalize'>${network}</span> Network. Note You Have Been Debited Of ₦${mainStore.addCommas(amount)}.`,
                    allowEscapeKey: false,
                    allowOutsideClick: false,
    ...
    <LayoutAuthenticated>
        ...
        <SectionMain>
            <SectionTitleLineWithButton :icon="mdiCellphoneWireless" :title="`Airtime Topup`" main>
                ...
                <BaseButton
                    :href="route('user_vtu_history_page') + '?length=10&type=airtime&isDirty=true&__rememberable=true'"
            ...
            <div>
                ...
                <CardBox v-if="!epins_received" isForm @submit.prevent="validateBuyAirtimeRequest" class="">
                    ...
                    <div class="mx-2 my-9">
                                ...
                                <div :class="buy_airtime_request.network == network.name ? 'selected' : ''"
                                    ...
                                    <div class="card-body text-center">
                                        <div v-if="network.discount > 0"
                                            class="bg-emerald-500 rounded-lg mb-2 text-xs px-1 py-1 font-semibold text-white">
                                            {{ network.discount
                                            }}% discount</div>
                        ...
                        <div class="row">
                            <div v-if="buy_airtime_request.errors.network" class="form-error">{{
                                buy_airtime_request.errors.network }}
                            </div>
                        </div>
                    </div>
                    ...
                    <FormField v-if="!epin_checkbox_checked" class="" label="">
                        <FormControl v-model="buy_airtime_request.entered_amount"
                            :error="buy_airtime_request.errors.amount" :icon="mdiCurrencyNgn" type="number"
                            id="entered_amount" placeholder="100-50,000" />
                    </FormField>
                    ...
                    <FormField v-if="!epin_checkbox_checked" class="" label="">
                        <FormControl v-model="buy_airtime_request.phone_number"
                            :error="buy_airtime_request.errors.phone_number" :icon="mdiPhone" type="text"
                            id="phone_number" placeholder="e.g 08127027321" />
                    </FormField>
              ...
              <label for="generate-epin-check" class="login-checkbox-label ">Generate E-PIN</label>
                    ...
                    <p id="epin-cashback" v-if="epin_checkbox_checked"><em class="text-emerald-500">{{ cashback_text }}</em>
                    </p>
                ...
                <CardBox v-else id="epin-card">
                    ...
                    <div class="">
                        ...
                        <h4 class="text-lg">Amount: <em class='text-emerald-500'
                                v-html="'₦' + mainStore.addCommas(epins_amount)"></em>
                            <br>Quantity: <em class='text-emerald-500' v-html='epins_quantity'></em> Unit(s)
                        </h4>
            ...
            <CardBoxModal v-model="showPreviewTransactionModal" button="danger" buttonLabel="Close"
                ...
                <div class="">
                    <div class="text-center">
                        <p>Kindly confirm that the details you entered are valid before clicking the "Confirm" button.
                        </p>
                            ...
                            <tbody>
                                <tr v-if="buy_airtime_request.recharge_type == 'epin'">
                                    ...
                                    <td><em v-html="buy_airtime_request.quantity + ' Unit(s)'"
                                            class="text-emerald-500"></em></td>
                                </tr>
                                <tr>
                                    <td>AMOUNT</td>
                                    <td><em v-html="`₦${mainStore.addCommas(buy_airtime_request.amount)}`"
                                            class="text-emerald-500"></em></td>
                                </tr>
                                <tr>
                                    <td>PHONE</td>
                                    <td><em v-html="buy_airtime_request.phone_number" class="text-emerald-500"></em></td>
                                </tr>
                                <tr>
                                    <td>NETWORK</td>
                                    <td><em class="text-emerald-500">{{ buy_airtime_request.network }}</em></td>
                                </tr>
                                <tr>
                                    <td>RECHARGE TYPE</td>
                                    <td><em class="text-emerald-500">{{ buy_airtime_request.recharge_type }}</em></td>
                                </tr>
                                <tr v-if="buy_airtime_request.recharge_type == 'normal'">
                                    <td>DISCOUNT</td>
                                    <td><em class="text-emerald-500">{{ network_discount }}%</em></td>
                                </tr>
                                <tr>
                                    <td>PAYABLE</td>
                                    <td><em v-html="`₦${mainStore.addCommas(buy_airtime_request.payable)}`"
                                            class="text-emerald-500"></em>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                    ...
                    <div class="justify-content-center text-center">
                        ...
                        <BaseButton @click="confirmAndProceedWithTransaction" class="" label="Confirm" color="success"
                            rounded />
                        <br>
                        <p class="text-red-500 cursor-pointer" @click="showPreviewTransactionModal = false">Cancel and
                            return</p>
                    </div>
                </div>
            ...
            <CardBoxModal v-model="showPreviewReloadlyTransactionModal" button="danger" buttonLabel="Close"
                ...
                <div class="">
                    ...
                    <div class="overflow-x-hidden max-h-[350px]">
                        <table class="table table-bordered mt-4">
                            <tbody>
                                ...
                                <tr>
                                    <td>AMOUNT</td>
                                    <td><em v-html="`₦${mainStore.addCommas(reloadly_airtime_request.amount)}`"
                                            class="text-emerald-500"></em>
                                    </td>
                                </tr>
                                <tr>
                                    <td>PHONE</td>
                                    <td><em v-html="reloadly_airtime_request.phone_number" class="text-emerald-500"></em>
                                    </td>
                                </tr>
                    ...
                    <div class="justify-content-center text-center">
                        ...
                        <BaseButton @click="confirmAndProceedWithReloadlyTransaction" class="" label="Confirm"
                            color="success" rounded />
                        <br>
                        <p class="text-red-500 cursor-pointer" @click="showPreviewReloadlyTransactionModal = false">
                            Cancel and
                            return</p>
                    </div>
                </div>
