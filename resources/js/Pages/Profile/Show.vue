      ...
      <div class="relative w-full h-full">
                        ...
                        <div class="-px-2 py-4">
                            <div class="">
                                <div class="w-full h-[300px] relative"
                                    :style="`background:linear-gradient(rgba(0, 0, 0,0.3),rgba(0, 0, 0,0.3)), url('${user.cover_photo}'); -webkit-background-size:cover; -moz-background-size:cover; background-size: cover; -o-background-size:cover; background-repeat: no-repeat; background-position: center;`">

                                    <div @click="changeProfilePicture" v-if="auth_user.id == user.id"
                                        :class="auth_user.id == user.id ? 'cursor-pointer' : ''"
                                        class="absolute w-[150px] h-[150px]  left-0 right-0 sm:mx-0 bottom-0 rounded-full sm:rounded-lg sm:ml-9 mx-auto bg-white p-[2px] -mb-[50px] z-[2]">
                                        <img :src="user.profile_picture"
                                            class="w-full h-full rounded-full sm:rounded-lg" alt="">

                                    </div>

                                    <div @click="changeCoverPhoto" v-if="auth_user.id == user.id"
                                        class="absolute top-0 right-0 mt-7 mr-6 bg-black opacity-70 rounded-full p-2 px-3 cursor-pointer"
                                        title="Change cover photo">
                                        <font-awesome-icon class="text-white font-bold text-xl "
                                ...
                                <div
                                    class=" bg-white dark:bg-slate-800 rounded-xl mx-4 -mt-9 z-[1] relative my-2 dark:text-slate-300">
                                    <div class="sm:ml-[200px]">
                                        <div class="h-[100px] sm:h-[10px]"></div>

                                        <div v-if="auth_user.id != user.id"
                                            class="mr-7 text-sm float-right sm:block hidden">
                                            <button @click="mainStore.toggleFollowUser(user)"
                                                :class="!user.following_loading ? 'bg-gradient-to-r from-primary-100 to-primary-200' : 'bg-white border-[1px] border-secondary-200'"
                                                class=" rounded-full text-center font-semibold py-3 px-9 text-white ">
                                                <img v-if="user.following_loading" class="w-[35px] mx-auto"
                                                    :src="Loader" alt="">

                                                <span v-else
                                                    v-text="user.following_status ? 'Following' : 'Follow'"></span>
                                            </button>
                                        </div>

                                        <div v-else class="mr-7 text-sm float-right sm:block hidden mt-3">
                                            <Link :href="route('edit_profile')"
                                                class="bg-gradient-to-r from-emerald-400 to-emerald-500 rounded-full text-center font-semibold py-3 px-9 text-white ">
                                        ...
                                        <h4 class="text-lg  font-semibold sm:text-left text-center text-gray-400 ">{{
                                            user.user_name }}</h4>
                                        <h4 class="text-2xl sm:text-xl font-bold sm:text-left text-center sm:mt-1 mt-1">
                                            {{ user.name }}</h4>

                                        <p class="text-xs leading-tight text-center sm:text-left my-2">{{ user.bio }}
                                        </p>
                                        <p class="text-sm text-gray-400 text-center sm:text-left my-2">
                                            <font-awesome-icon class="text-red-400" icon="fa-solid fa-location-dot" />
                                            {{ user.lga_details.name }}, {{ user.state_details.name }}, Nigeria
                                        </p>
                                        <div v-if="auth_user.id != user.id"
                                            class="text-lg my-3 text-center block sm:hidden">
                                            <button @click="mainStore.toggleFollowUser(user)"
                                                :class="!user.following_loading ? 'bg-gradient-to-r from-primary-100 to-primary-200' : 'bg-white border-[1px] border-secondary-200'"
                                                class=" rounded-full text-center font-semibold py-2 px-9 text-white ">
                                                <img v-if="user.following_loading" class="w-6 mx-auto" :src="Loader"
                                                    alt="">
                                    ...
                                    <div
                                        class="flex justify-center items-center my-6 mb-3 bg-gray-100 dark:bg-slate-600 mx-2 rounded-xl px-3 py-2">
                                        <div class="text-center">
                                            <div
                                                class="inline-block mx-3 p-2 px-4 border-4 border-primary-200 rounded-xl cursor-pointer">
                                                <h4 class="text-3xl font-bold">{{
                                                    mainStore.convertNumberToKOrMorB(user.total_posts_num) }}</h4>
                                                <h6 class="text-sm font-bold">Posts</h6>
                                            </div>

                                            <div @click="mainStore.loadUserFollowers(user.slug)"
                                                class="inline-block mx-3 cursor-pointer">
                                                <h4 class="text-3xl font-bold">{{
                                                    mainStore.convertNumberToKOrMorB(user.followers_num) }}</h4>
                                                <h6 class="text-sm font-bold">Followers</h6>
                                            </div>

                                            <div @click="mainStore.loadUserFollowing(user.slug)"
                                                class="inline-block mx-3 cursor-pointer">
                                                <h4 class="text-3xl font-bold">{{
                                                    mainStore.convertNumberToKOrMorB(user.following_num) }}</h4>
                                    ...
                                    <div
                                        class="bg-gray-100 dark:bg-slate-600 mx-2 px-3 py-2 rounded-xl posts mt-6 text-secondary-200 dark:text-slate-300 ">

                                        <div class="sm:w-6/12 flex justify-center mx-auto my-4"
                                            v-if="auth_user.id == user.id">
                                            <CreatePost :placeholder="`What's on your mind ${auth_user.name}?`" />
                                        </div>

                                        <div class="flex justify-center space-x-12  mt-[60px] pt-2">
                                            <!-- Posts Tab -->
                                            <button @click="activeTab = 'posts'"
                                                :class="activeTab === 'posts' ? 'text-black border-t-2 border-black' : 'text-gray-400'"
                                                class="flex items-center py-2 text-sm font-semibold">
                                            ...
                                            <button @click="activeTab = 'reels'"
                                                :class="activeTab === 'reels' ? 'text-black border-t-2 border-black' : 'text-gray-400'"
                                                class="flex items-center py-2 text-sm font-semibold">
                                        ...
                                        <transition name="fade" mode="out-in">
                                            <div v-if="activeTab === 'posts'" class="">
                                                ...
                                                <div v-if="posts.length > 0" class="mt-[40px]">
                                                    ...
                                                    <div class="sm:grid sm:grid-cols-12 sm:gap-6">
                                                        <Post class="sm:col-span-6" v-for="(post, index) in posts"
                                                            :key="index" :post="post" :index="index"
                                                            :make_comment_open="make_comment_open"
                                                            @open-bigger-post="openBiggerPost"
                                                            @make-comment-action="makeCommentAction"
                                            ...
                                            <div v-else-if="activeTab === 'reels'" class="mt-6">
                                                <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                                    <div v-for="(reel, index) in reels" :key="index"
                                                        class="relative group bg-black overflow-hidden">

                                                        <!-- 🎥 Video itself -->
                                                        <video :src="reel.media[0].path"
                                                            class="w-full h-full object-cover aspect-square" muted loop
                                                            playsinline></video>

                                                        <!-- ✅ Likes bottom-left -->
                                                        <div class="absolute bottom-2 left-2 flex items-center space-x-1 text-white text-xs font-semibold 
               opacity-100 group-hover:opacity-0 transition-opacity duration-300">
                                                            <font-awesome-icon icon="fa-solid fa-heart"
                                                                class="text-sm" />
                                                            <span>{{ reel.likes_num }}</span>
                                                        </div>

                                                        <!-- ✅ Comments bottom-right -->
                                                        <div class="absolute bottom-2 right-2 flex items-center space-x-1 text-white text-xs font-semibold 
               opacity-100 group-hover:opacity-0 transition-opacity duration-300">
                                                            <font-awesome-icon icon="fa-solid fa-comments"
                                                                class="text-sm" />
                                                            <span>{{ reel.comments_num }}</span>
                                                        </div>

                                                        <!-- 🔥 Hover overlay (likes & comments center) -->
                                                        <div class="absolute inset-0 bg-black bg-opacity-40 flex items-center justify-center 
               opacity-0 group-hover:opacity-100 transition-opacity duration-300">

                                                            <div
                                                                class="flex space-x-6 text-white text-lg font-semibold">
        ...
        <CardBoxModal v-model="showUpdateCoverPhotoModal" button="danger" buttonLabel="Close"
            ...
            <form @submit.prevent="submitCoverPhotoForm" class="mt-5">
                <h6 class="text-sm italic font-semibold my-2 text-primary-100">Select an image to upload. Max size: 3MB
                </h6>
                <div @click="selectCoverPhotoFiles"
                    class="w-full h-[200px] relative border rounded border-gray-300  cursor-pointer">
                    <div class="absolute top-0 bottom-0 right-0 left-0 m-auto h-[80px] w-[80px]  text-gray-400">
                        ...
                        <h5 class="text-lg font-semibold"
                            v-text="cover_photo_form.image ? 'Change image' : 'Select an image'"></h5>
                    </div>
                </div>
                <input type="file" class="hidden" ref="post_medias" accept=".jpg, .png, .jpeg"
                ...
                <div v-if="cover_photo_form.image"
                    class="text-xs py-2 text-secondary-400 dark:text-slate-600 max-h-[140px] overflow-x-hidden">
                    <h4 class="text-2xl font-bold text-center">Preview</h4>

                    <img v-if="cover_photo_form.image.type == 'image/jpeg' || cover_photo_form.image.type == 'image/jpg' || cover_photo_form.image.type == 'image/png'"
                        :src="cover_photo_form.image.preview" class="inline-block mr-1" alt="">

                </div>
                <div :style="`width: ${cover_photo_form.progress ? cover_photo_form.progress.percentage : 0}%`"
                    class="h-1 bg-gradient-to-r from-primary-100 via-purple-400 to-primary-200 my-2"></div>
                <button class="form-button"
                    :class="cover_photo_form.processing ? 'opacity-60 cursor-not-allowed' : ''">Upload</button>
            </form>
        </CardBoxModal>
        ...
        <CardBoxModal v-model="showUpdateProfilePhotoModal" button="danger" buttonLabel="Close"
            ...
            <form @submit.prevent="submitProfilePictureForm" class="mt-5">
                ...
                <div @click="selectProfilePictureFiles"
                    class="w-full h-[200px] relative border rounded border-gray-300  cursor-pointer">
                    <div class="absolute top-0 bottom-0 right-0 left-0 m-auto h-[80px] w-[80px]  text-gray-400">
                        <font-awesome-icon class="text-3xl" icon="fa-solid fa-file-circle-plus" />
                        <h5 class="text-lg font-semibold"
                            v-text="profile_picture_form.image ? 'Change image' : 'Select an image'"></h5>
                    </div>
                </div>
                <input type="file" class="hidden" ref="profile_photo_medias" accept=".jpg, .png, .jpeg"
                ...
                <div v-if="profile_picture_form.image"
                    class="text-xs py-2 text-secondary-400 dark:text-slate-600 max-h-[140px] overflow-x-hidden">
                    <h4 class="text-2xl font-bold text-center">Preview</h4>

                    <img v-if="profile_picture_form.image.type == 'image/jpeg' || profile_picture_form.image.type == 'image/jpg' || profile_picture_form.image.type == 'image/png'"
                        :src="profile_picture_form.image.preview" class="inline-block mr-1" alt="">

                </div>
                <div :style="`width: ${profile_picture_form.progress ? profile_picture_form.progress.percentage : 0}%`"
                    class="h-1 bg-gradient-to-r from-primary-100 via-purple-400 to-primary-200 my-2"></div>
                <button class="form-button"
                    :class="profile_picture_form.processing ? 'opacity-60 cursor-not-allowed' : ''">Upload</button>
            </form>
        </CardBoxModal>

        <PostPopup v-model:open-post-index="open_post_index" v-model:open-media-index="open_media_index"
            v-model:current-media-index="current_media_index"
            :post="open_post_index !== false && open_media_index !== false ? posts[open_post_index] : {}"
