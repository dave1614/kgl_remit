...
const unread_notifications_num = page.props.unread_notifications_num;

// console.log(unread_notifications_num)
...
const itemLabel = computed(() => props.item.isCurrentUser ? usePage().props.auth.user.user_name : props.item.label)
...
<template>
    ...
    <component :is="is" v-else ref="root" class="block lg:flex items-center relative cursor-pointer"
        :class="componentClass" :href="itemHref" @click="menuClick">
        <div class="flex items-center" :class="{
            ...
            <span v-if="item.label === 'Notifictaions' && unread_notifications_num > 0"
                class="absolute top-1 right-0 bg-red-500 text-white text-[10px] font-bold rounded-full px-1.5 py-0.5 min-w-[18px] text-center">
                {{ unread_notifications_num > 9 ? '9+' : unread_notifications_num }}
            </span>
